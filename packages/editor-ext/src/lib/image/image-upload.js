...
exports.handleImageUpload = exports.ImageUploadPlugin = void 0;
const state_1 = require("@tiptap/pm/state");
const view_1 = require("@tiptap/pm/view");
const media_utils_1 = require("../media-utils");
const uploadKey = new state_1.PluginKey("image-upload");
const ImageUploadPlugin = ({ placeholderClass, }) => new state_1.Plugin({
    key: uploadKey,
    state: {
        init() {
...
exports.ImageUploadPlugin = ImageUploadPlugin;
function findPlaceholder(state, id) {
    const decos = uploadKey.getState(state);
    const found = decos.find(undefined, undefined, (spec) => spec.id == id);
    return found.length ? found[0]?.from : null;
}
const handleImageUpload = ({ validateFn, onUpload }) => async (file, view, pos, pageId) => {
